...
int main(){
	...
	fbo.create();
	fbo.bind();
	fbo.attach(sampleTexture,gl::FBOAttachment::Color0);
	if(!fbo.complete())printf("Framebuffer not complete.\n");
	fbo.unbind();
	...
	while(window.isOpen()){
		...
		sampleShader.setVec2("windowSize",window.width,window.height);
		...
		sampleShader.setInt("numFrames",frameCounter);
